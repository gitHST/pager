...
class SearchBooksSmartTest {
    ...
    @Test
    fun `searchBooksSmart returns combined deduplicated sorted books`() =
        runTest {
            val titleBooks =
                listOf(
    ...
    @Test
    fun `searchBooksSmart handles HttpException 503`() =
        runTest {
            coEvery { OpenLibraryService.api.searchBooks(any(), any()) } throws
                HttpException(
                    Response.error<Any>(
                        503,
                        "".toResponseBody(null),
    ...
    @Test
    fun `searchBooksSmart handles IOException`() =
        runTest {
