...
class SearchLogicTest {
    @Test
    fun `searchBooksSmart returns correct result after delay`() =
        runTest {
    ...
    @Test
    fun `searchBooksSmart returns error message`() =
        runTest {
            val mockSearchBooks: suspend (String) -> SearchResult = mockk()
            val query = "errorTest"
    ...
    @Test
    fun `debounced logic cancels previous search if query changes`() =
        runTest {
